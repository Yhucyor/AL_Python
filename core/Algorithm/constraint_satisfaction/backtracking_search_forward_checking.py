...
def _backtracking_search_forward_checking(csp):
    return backtrack({}, csp)

def backtracking_search_forward_checking(problem_matrix):
    csp = Puzzle_CSP(problem_matrix, max_depth = 15)
    assignment = _backtracking_search_forward_checking(csp)
    if assignment:
...
def backtrack(assignment, csp):
    ...
    for value in csp.domain_values(var, assignment):
        if csp.is_consistent(var, value, assignment):
            ...
            # Forward checking
            if csp.forward_check(var, value, assignment):
                #Chỉ đếm khi mở 
                g.tk_da_duyet += 1
